...
export default function RelaxationScreen({ activity, onBack, onComplete }) {
  ...
  const next = () => {
    if (stepIdx === -1) {
      ...
      setDone(true)
      stop()
      if (activity.closing) speak(activity.closing)
    } else {
      const n = stepIdx + 1
      setStepIdx(n)
      speak(steps[n].body)
    ...
    <div style={{ minHeight: '100dvh', background: C.bg, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '1rem 1.25rem', borderBottom: `0.5px solid ${C.border}`, background: C.card }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '12px', color: C.muted }}>{activity.duration} min · Relaxation</div>
        </div>
        {stepIdx >= 0 && (
      ...
      {stepIdx >= 0 && (
        <div style={{ height: '2px', background: C.border }}>
          <div style={{ height: '100%', background: C.primary, width: `${((stepIdx + 1) / steps.length) * 100}%`, transition: 'width 0.3s' }} />
        </div>
      )}
        ...
        {done ? (
          <>
            ...
            <p style={{ fontSize: '15px', color: C.muted, lineHeight: 1.6, marginBottom: '2rem' }}>
              {activity.closing ?? 'Well done. Take a moment to notice how different your body feels.'}
            </p>
            <button onClick={() => { onComplete?.(); onBack() }} style={btnStyle(C.primary, '#fff')}>Done</button>
          </>
        ) : stepIdx === -1 ? (
          <>
            <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: C.primaryLight, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.primary} strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
            ...
            <p style={{ fontSize: '14px', color: C.muted, lineHeight: 1.6, marginBottom: '2rem' }}>
              Find a comfortable place to sit or lie down. Each step will be read aloud — follow along at your own pace.
            </p>
            <button onClick={next} style={btnStyle(C.primary, '#fff')}>Begin</button>
          </>
        ) : (
          <>
            <div style={{ background: C.primaryLight, borderRadius: '99px', padding: '4px 14px', fontSize: '12px', fontWeight: 500, color: C.primary, marginBottom: '2rem' }}>
              {current.heading}
            </div>
            <p style={{ fontSize: '17px', lineHeight: 1.75, color: C.text, marginBottom: '2.5rem', maxWidth: '360px' }}>
              {current.body}
            </p>
            <button onClick={next} style={btnStyle(C.primary, '#fff')}>
